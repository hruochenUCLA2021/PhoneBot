...
class TfliteActor:
    ...
    def run(self, state_1d):
        ...
        # TFLite expects [1, state_dim]
        import numpy as np  # local import to avoid hard requirement at module import
        ...
        y = y.reshape((-1,))
        if y.size != self.action_dim:
            raise ValueError(f"action size {y.size} != expected {self.action_dim}")
...
def build_phonebot_obs52(
    *,
    ...
    # gyro: trunk -> (base if normal) -> imu_site
    ...
    # gravity: reconstruct q_imu_in_world
...
class PcPolicyRunner(Node):
    def __init__(self):
        super().__init__("pc_policy_runner")
        ...
        if self._meta.state_dim != 52 or self._meta.action_dim != 13:
            self.get_logger().warn(
                f"Policy dims: state_dim={self._meta.state_dim} action_dim={self._meta.action_dim} "
                "(this runner is written for 52->13; may fail if different)"
            )
        ...
        self._sub_motor = self.create_subscription(MotorState, motor_state_topic, self._on_motor_state, 10)
        self._sub_cmd = self.create_subscription(Twist, cmd_vel_topic, self._on_cmd_vel, 10)
        self._pub_motor_cmd = self.create_publisher(JointState, motor_cmd_topic, 10)

        period = 1.0 / max(1.0, self._publish_hz)
        self._timer = self.create_timer(period, self._on_timer)
        self.get_logger().info(
            f"pc_policy_runner ready: publish_hz={self._publish_hz:.1f} "
    ...
    def _on_timer(self):
        ...
        # Command timeout: if joystick/keyboard stops, command goes to zero.
        if cmd_age > 1.0:
            cmd_vx = 0.0
            cmd_vy = 0.0
            cmd_wz = 0.0
        ...
        # IMU: ROS gives (x,y,z,w); we convert to (w,x,y,z)
        q = imu.orientation
        ...
        # Phase update (use real dt between timer ticks)
        now_ns = time.time_ns()
        ...
        # Update last_act and map to targets: default_pose + action (action_scale=1.0), then clip.
        tgt = [0.0] * 13
        for i in range(13):
